...
var handler = async (event, context) => {
  const { query, pageNumber, pageSize } = event.queryStringParameters;
  const fetchUSDAdata = await fetch(`https://api.nal.usda.gov/fdc/v1/foods/search?api_key=${process.env.USDA_API_KEY}`, {
    method: "POST",
    headers: {
  ...
  const foods = searchResponse.foods.map((food) => {
    ...
    return {
      ...
      foodNutrients: (_a4 = food.foodNutrients) == null ? void 0 : _a4.map(({
        nutrientName,
        unitName,
        value,
